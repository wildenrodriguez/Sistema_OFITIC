...
if (is_file("view/".$page.".php")) {
	...
	if (isset($_POST["registrar"])) {
		...
		$msg = "(".$_SESSION['user']['nombre_usuario']."), Se registró un material: ";
		$hora = date('H:i:s');
		$fecha = date('Y-m-d');
	...
	if (isset($_POST['consultar'])) {
		$material->set_cedula_solicitante($_SESSION['user']['cedula']);
	...
	if (isset($_POST["actualizar"])) {
		$material->set_cedula_solicitante($datos["cedula"]);
		$material->set_motivo($_POST["motivo"]);
		$peticion["peticion"] = "registrar";
		echo json_encode($material->Transaccion($peticion));
